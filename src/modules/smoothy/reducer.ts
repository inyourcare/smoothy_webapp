...
import {
  activateEachScreenHammerMode,
  attachPingListAction,
  clearFCMMessageAction,
  clearPingListAction,
  clearPlaybackAction,
  clearPlaylistAction,
  clearRoomConnectedAction,
  clearSelectedVideoAction,
  clearToBeUserAction,
  clearTwilioVideochatPropsAction,
  deactivateEachScreenHammerMode,
  loadDoneToBeUserAction,
  setAlertSnackbarAction,
  setButtonDisableAction,
  setButtonEnableAction,
  setFCMMessageAction,
  setMyProfileAction,
  setPlaybackAction,
  setPlaylistAction,
  setReactionActivated,
  setRoomConnectedAction,
  setSelectedVideoAction,
  setToBeUserAction,
  setTwilioVideochatPropsAction,
  setVideochatUserProfilesAction,
  setYoutubeDivWidthHeightAction,
} from "./actions";
...
const smoothy = createReducer<SmoothyState, SmoothyAction>(initialState)
  ...
  .handleAction(loadDoneToBeUserAction, (state) => {
    return {
      ...state,
      toBeUser: { ...state.toBeUser, loading: false } as ToBeUser,
    };
  })
  ...
  .handleAction(attachPingListAction, (state, action) => {
    ...
    if (pingListMap.has(payload.partyNo)) {
      ...
      const filtered = list?.filter(
        (prevState) => prevState.uid === payload.uid
      );
      filtered
        ? filtered.forEach((newState) => {
  ...
  .handleAction(setReactionActivated, (state, action) => {
    return { ...state, reactionActivated: action.payload };
  });
